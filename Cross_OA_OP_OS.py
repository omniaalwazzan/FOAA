...
class Cross_OA_OP_OS(nn.Module):
    def __init__(self,atttention_OA,atttention_OP,atttention_OS,model_image,model_EHR,nb_classes=2, h_dim=1):
        super(Cross_OA_OP_OS, self).__init__()
    ...
    def forward(self, x1,x2):
        ...
        ### 5) Aggregate enhanced features 
        x = torch.sum(torch.stack([I_prim_add,EHR_prim_add,I_prim_prod,EHR_prim_prod,I_prim_sub,EHR_prim_sub,x1,x2]), dim=0) # This operation will perform element-wise addition, maintaining the original size of the vector.
...
model = Cross_OA_OP_OS(atttention_OA,atttention_OP,atttention_OS,img,ehr).to(device=DEVICE,dtype=torch.float)
